...
class M_list_bayar_tahun extends CI_Model{
	private $dbOracle;
	...
	public function generateReport($PeriodeAwal, $PeriodeAkhir, $KodeKecamatan, $KodeKelurahan, $Tahun, $JenisBuku){
		if($JenisBuku=='0'){
			$JenisBuku = "(PBS.JML_SPPT_YG_DIBAYAR - PBS.DENDA_SPPT) > 0";
		}elseif($JenisBuku=='1'){
			$JenisBuku = "(PBS.JML_SPPT_YG_DIBAYAR - PBS.DENDA_SPPT) < 2000000";
		}else{
			$JenisBuku = "(PBS.JML_SPPT_YG_DIBAYAR - PBS.DENDA_SPPT) >= 2000000";
		}
	...
	AND PBS.THN_PAJAK_SPPT = '$Tahun'
	AND PBS.KD_PROPINSI||'.'||PBS.KD_DATI2||'.'||PBS.KD_KECAMATAN||'.'||PBS.KD_KELURAHAN = '$KodeKelurahan'
	AND $JenisBuku
	ORDER BY PBS.KD_PROPINSI, PBS.KD_DATI2, PBS.KD_KECAMATAN, PBS.KD_KELURAHAN, PBS.KD_BLOK, PBS.NO_URUT, PBS.KD_JNS_OP, PBS.THN_PAJAK_SPPT";
